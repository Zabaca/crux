...
- `./.fredrin/fredrin ticket context-classify [--notes "..."]` — **advisory** significance gate:
  ...
  (`routine` → no ADR; `significant` → add one under `.fredrin/memory/adr/`, and
  update the concept doc under `.fredrin/memory/concepts/`). Never blocks the build.
...
`AGENTS.md`, and the **Memory bundle under `.fredrin/memory/`** (concept docs in
`.fredrin/memory/concepts/`, decision records/ADRs in `.fredrin/memory/adr/`) —
not here. Fredrin's Memory tab reads this bundle by default; write context there,
**not** in a top-level `docs/` folder, unless you've pointed the project's context
root elsewhere in project settings. The one exception is **Fredrin platform
