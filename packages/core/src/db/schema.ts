...
export const eliminationSolutions = sqliteTable(
  ...
  {
    eliminationId: text("elimination_id").notNull().references(() => eliminations.id),
...
export const decisions = sqliteTable("decisions", {
  ...
  problemId: text("problem_id").notNull().references(() => problems.id),
  chosenSolutionId: text("chosen_solution_id").notNull().references(() => solutions.id),
  rationale: text("rationale").notNull(),
  context: text("context"),
  decidedById: text("decided_by_id").notNull().references(() => users.id),
...
export const abandonments = sqliteTable("abandonments", {
  ...
  problemId: text("problem_id").notNull().references(() => problems.id).unique(),
  rationale: text("rationale").notNull(),
  abandonedById: text("abandoned_by_id").notNull().references(() => users.id),
  abandonedAt: integer("abandoned_at").notNull().default(sql`(unixepoch() * 1000)`),
});
...
export const outcomes = sqliteTable("outcomes", {
  ...
  observedAt: integer("observed_at").notNull().default(sql`(unixepoch() * 1000)`),
});
