...
describe("getAllowedActions", () => {
  ...
  test("problem_detail allows ADD_ATTEMPT and ABANDON_PROBLEM", () => {
    const a = getAllowedActions({ viewing: "problem_detail" });
    expect(a.allowedMutation).toContain("ADD_ATTEMPT");
...
describe("isActionAllowed", () => {
  ...
  test("ADD_ATTEMPT not allowed from workstream_dashboard", () => {
    expect(isActionAllowed("ADD_ATTEMPT", { viewing: "workstream_dashboard" })).toBe(false);
  });

  test("ADD_ATTEMPT allowed from problem_detail", () => {
    expect(isActionAllowed("ADD_ATTEMPT", { viewing: "problem_detail" })).toBe(true);
  });
});
