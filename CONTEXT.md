...
- **Root** — the Principal at the head of a linked set: itself when unclaimed, and whoever claimed it when not. Claiming refuses an already-claimed Principal, so the edge is always one hop and every corpus has exactly one. It is what the things that cannot take a set are keyed by — the view-state Durable Object is addressed by one id, not by the set (ADR-0014).
