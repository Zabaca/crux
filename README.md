...
- [`packages/cli`](packages/cli) — `crux` binary, command dispatch via citty, the HTTP client every command goes through, the Zod schemas its `--json` output is checked against, and the `config.toml` loader.
